...
### [ ] [v7.1.45-s15] (待交付 ⏳) - 提供商配置列表优先级降序排列修复 (TASK-CPA-FIX-PROVIDER-PRIORITY-SORT)
- **问题根因**：CPA Web 管理面板【AI 提供商】列表中，提供商排序逻辑与调度物理规则相反。系统底层路由是优先级数值越高越优先调度（`99 > 98 > 97 > ...`），但在管理界面中采用了升序排列，导致低优先级（如 97、98）被错误排在高优先级（如 99）上方。
- **修复与迭代规划**：
  1. **前端/API 列表排序矫正**：在管理控制台前端渲染与后端提供商列表响应处，强制执行 `priority` **降序排序 (`b.priority - a.priority`)**；
  2. **默认兜底与平齐**：未配置 priority 的提供商默认视为 0 沉底；同优先级按添加时间/名称稳定排序；
  3. **实时响应与刷盘**：修改优先级后即时按降序重排界面，并触发 `f.Sync()` 物理刷写 `config.yaml`。
